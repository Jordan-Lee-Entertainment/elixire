...
async def check_exists(test_cli, shortname, utoken, not_exists=False):
    ...
    resp = await test_cli.get("/api/list?page=0", headers={"Authorization": utoken})
...
async def _test_delete_file(test_cli):
    ...
    # TODO file
    resp = await test_cli.post("/api/upload", headers={"Authorization": utoken})
    ...
    resp_del = await test_cli.delete(
        f"/api/files/{short}", headers={"Authorization": utoken}
    )
...
async def test_delete_nonexist(test_cli):
    ...
    resp_del = await test_cli.delete(
        f"/api/files/{rand_file}", headers={"Authorization": utoken}
    )

    assert resp_del.status_code == 404

    # ensure sharex compatibility endpoint works too
    resp_del = await test_cli.get(
        f"/api/files/{rand_file}/delete", headers={"Authorization": utoken}
    )
