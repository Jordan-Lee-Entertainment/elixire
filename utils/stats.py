...
async def main():
    ...
    nd_file_count_c = await db.fetchval(
        f"""
    SELECT COUNT(*)
    FROM files
    {files_user_join}
    ...
    d_file_count_c = await db.fetchval(
        f"""
    SELECT COUNT(*)
    FROM files
    {files_user_join}
    ...
    nd_shorten_count_c = await db.fetchval(
        f"""
    SELECT COUNT(*)
    FROM shortens
    {shorten_user_join}
    ...
    d_shorten_count_c = await db.fetchval(
        f"""
    SELECT COUNT(*)
    FROM shortens
    {shorten_user_join}
    ...
    total_nd_file_size_c = await db.fetchval(
        f"""
    SELECT SUM(file_size)
    FROM files
    {files_user_join}
    ...
    total_d_file_size_c = await db.fetchval(
        f"""
    SELECT SUM(file_size)
    FROM files
    {files_user_join}
    ...
    nd_file_count_week_c = await db.fetchval(
        f"""
    SELECT COUNT(*)
    FROM files
    {files_user_join}
    ...
    d_file_count_week_c = await db.fetchval(
        f"""
    SELECT COUNT(*)
    FROM files
    {files_user_join}
    ...
    total_nd_file_size_week_c = await db.fetchval(
        f"""
    SELECT SUM(file_size)
    FROM files
    {files_user_join}
    ...
    total_d_file_size_week_c = await db.fetchval(
        f"""
    SELECT SUM(file_size)
    FROM files
    {files_user_join}
    ...
    nd_shorten_count_week_c = await db.fetchval(
        f"""
    SELECT COUNT(*)
    FROM shortens
    {shorten_user_join}
    ...
    d_shorten_count_week_c = await db.fetchval(
        f"""
    SELECT COUNT(*)
    FROM shortens
    {shorten_user_join}
