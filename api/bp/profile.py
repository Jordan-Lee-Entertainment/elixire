...
async def change_profile(request):
    ...
    if password and new_username is not None:
        ...
        await request.app.storage.raw_invalidate(
            f"uid:{old_username}", f"uname:{user_id}"
        )
    ...
    return response.json({"updated_fields": updated})
...
async def delete_file_task(app, user_id: int, delete=False):
    ...
    for row in file_shortnames:
        ...
        task = app.loop.create_task(_delete_file_wrapper(app, shortname, user_id))
        tasks.append(task)
...
"""
    ...
    resp, _ = await send_email(
        request.app, user_email, f"{_inst_name} - password reset request", email_body
    )
